...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (fetchError || !review) {
      return new NextResponse(
        renderPage('תגובה לא נמצאה', 'לא נמצאה תגובה עם המזהה שבקישור.', false),
    ...
    if (review.status === 'approved') {
      return new NextResponse(
        renderPage('כבר אושרה', `התגובה של ${review.name} כבר מפורסמת באתר.`, true),
    ...
    return new NextResponse(
      renderPage('אושר בהצלחה!', `התגובה של ${review.name} אושרה ותופיע עכשיו באתר.`, true),
      { headers: { 'Content-Type': 'text/html; charset=utf-8' } }
    );
  } catch (error) {
    const message = error instanceof Error ? error.message : 'שגיאה באישור התגובה';
