...
type DressImageFillProps = {
  ...
  /** contain = בתוך מסגרת קבועה. cover = חיתוך. natural = התמונה במלואה והמסגרת מתאימה אליה. smart = כל התמונה + רקע מטושטש + מיקוד על השמלה */
  ...
  return (
    <div className={`relative overflow-hidden bg-[#ebe4d6] ${className ?? ''}`}>
      ...
      <img
        ...
        loading={loading}
        draggable={false}
        className={`absolute inset-0 z-[1] h-full w-full object-contain ${imageClassName ?? ''} ${mainHoverClass}`}
        style={{ objectPosition: focus.objectPosition }}
      />
    </div>
  );
