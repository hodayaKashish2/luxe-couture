...
export default function CatalogPlatformNotice() {
  const { headline, subline, termsLabel } = CATALOG_PLATFORM_NOTICE;

  return (
    <div className="mb-3 rounded-xl border border-[#e6c687]/45 bg-[#fffdf9]/90 px-3 py-2 sm:px-4 sm:py-2.5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-3 text-center sm:text-right">
        ...
        <Link
          href="/terms"
          className="shrink-0 text-[10px] sm:text-[11px] font-bold text-[#b8860b] underline underline-offset-2 hover:text-[#8b6508] whitespace-nowrap"
        >
          {termsLabel} →
        </Link>
      </div>
